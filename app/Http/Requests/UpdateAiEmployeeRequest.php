...
class UpdateAiEmployeeRequest extends FormRequest
{
    public function authorize(): bool { return true; }

    public function rules(): array
    {
        $id = $this->route('aiEmployee')?->id ?? $this->route('ai_employee');

        return [
            'name'                      => 'sometimes|required|string|max:100',
            'slug'                      => "sometimes|nullable|string|max:100|unique:ai_employees,slug,{$id}",
            'role_key'                  => 'sometimes|required|string|max:60',

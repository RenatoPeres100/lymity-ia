    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        <div class="table-wrapper" style="padding:28px;">
            <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;">
                ...
                <div>
                    <div style="font-size:1.1rem;font-weight:700;color:#f1f5f9;">{{ $employee->name }}</div>
            ...
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #0f172a;">
                        <td style="padding:10px 0;font-size:.8rem;color:#e2e8f0;">{{ $task->title }}</td>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        <div class="table-wrapper" style="padding:24px;">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                ...
                <div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:#0f172a;border-radius:8px;border:1px solid #1e293b;">
                    @if($skill->icon)<span style="font-size:1rem;">{{ $skill->icon }}</span>@endif
                    <div>
                        <div style="font-size:.8rem;font-weight:600;color:#e2e8f0;">{{ $skill->name }}</div>
                        @if($skill->category)<div style="font-size:.7rem;color:#475569;">{{ $skill->category }}</div>@endif
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="table-wrapper" style="padding:24px;">
            ...
            <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #0f172a;">
                <span style="font-size:.8rem;color:#94a3b8;">{{ $label }}</span>
                <span style="font-size:.875rem;font-weight:700;color:#e2e8f0;">{{ $count }}</span>
            </div>
            @endforeach
        </div>
    </div>

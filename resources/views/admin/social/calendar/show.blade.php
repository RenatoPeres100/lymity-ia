    ...
    <div style="padding:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:2rem;">
            <div>
                ...
                <h1 style="font-size:1.8rem;font-weight:700;color:#0f172a;margin-top:.5rem;">
            ...
            <form method="POST" action="{{ route('admin.social.calendar.destroy', $calendar) }}" onsubmit="return confirm('Excluir calendário?')">
                @csrf @method('DELETE')
                <button style="background:#fef2f2;color:#dc2626;border:1px solid #fca5a5;padding:.5rem 1rem;border-radius:.375rem;cursor:pointer;font-size:.85rem;">Excluir</button>
            </form>
        </div>
        ...
        <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-bottom:1.5rem;">
            ...
            <p style="color:#cbd5e1;font-size:.9rem;line-height:1.7;">{{ $calendar->strategy_summary }}</p>
        </div>
        @endif
        ...
        <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-top:1.5rem;">
            <h3 style="color:#e2e8f0;font-weight:600;margin-bottom:1rem;">Posts do Mês ({{ $posts->count() }})</h3>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem 0;border-bottom:1px solid #334155;">
                <div>
                    <a href="{{ route('admin.social.posts.show', $post) }}" style="color:#93c5fd;text-decoration:none;font-size:.9rem;">{{ $post->title }}</a>

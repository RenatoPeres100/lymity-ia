    ...
    <div style="padding:2rem;max-width:900px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:2rem;">
            ...
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
                ...
                <form method="POST" action="{{ route('admin.social.posts.send-approval', $post) }}" style="display:inline;">
                    @csrf
                    <button style="background:#f59e0b;color:#000;padding:.5rem .9rem;border-radius:.375rem;border:none;cursor:pointer;font-size:.85rem;">Enviar Aprovação</button>
                </form>
                @endif
                ...
                @endif
                <a href="{{ route('admin.social.posts.edit', $post) }}" style="background:#334155;color:#94a3b8;padding:.5rem .9rem;border-radius:.375rem;text-decoration:none;font-size:.85rem;">Editar</a>
                <a href="{{ route('admin.social.ai.improve', $post) }}" style="background:#8b5cf620;color:#8b5cf6;border:1px solid #8b5cf6;padding:.5rem .9rem;border-radius:.375rem;text-decoration:none;font-size:.85rem;">✨ Melhorar IA</a>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:2fr 1fr;gap:1.5rem;">
            <div>
                ...
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-bottom:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:1rem;">Legenda Principal</h3>
                    @if($post->main_caption)
                    <p style="color:#f1f5f9;font-size:.95rem;line-height:1.7;white-space:pre-wrap;">{{ $post->main_caption }}</p>
                    @else
                    <p style="color:#64748b;font-size:.9rem;">Sem legenda gerada.</p>
                    @endif
                    @if($post->hashtags)
                ...
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-bottom:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:1rem;">Variações por Plataforma</h3>
                    @foreach($post->variants as $variant)
                    <div style="border:1px solid #334155;border-radius:.5rem;padding:1rem;margin-bottom:1rem;">
                        <p style="color:#f1f5f9;font-size:.9rem;font-weight:600;margin-bottom:.5rem;">{{ strtoupper($variant->platform) }}</p>
                        <p style="color:#94a3b8;font-size:.85rem;line-height:1.6;white-space:pre-wrap;">{{ $variant->caption }}</p>
                        @if($variant->creative_notes)
                        <p style="color:#64748b;font-size:.75rem;margin-top:.5rem;font-style:italic;">{{ $variant->creative_notes }}</p>
                        @endif
                    </div>
                    @endforeach
                </div>
                ...
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:1rem;">Gerar Variações</h3>
            ...
            <div>
                ...
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-bottom:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:1rem;">Agendar</h3>
                    <form method="POST" action="{{ route('admin.social.posts.schedule', $post) }}">
                        @csrf @method('PATCH')
                        <input type="datetime-local" name="scheduled_at" required style="width:100%;background:#0f172a;border:1px solid #334155;color:#f1f5f9;padding:.5rem;border-radius:.375rem;font-size:.85rem;margin-bottom:.75rem;">
                ...
                {{-- Info --}}
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;margin-bottom:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:1rem;">Informações</h3>
                    @foreach([
                ...
                <div style="background:#1e293b;border:1px solid #334155;border-radius:.75rem;padding:1.5rem;">
                    <h3 style="color:#94a3b8;font-size:.85rem;font-weight:600;text-transform:uppercase;margin-bottom:.75rem;">Brief Criativo</h3>
                    <p style="color:#94a3b8;font-size:.85rem;line-height:1.6;">{{ $post->creative_brief }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div style="margin-top:1.5rem;padding-top:1.5rem;border-top:1px solid #334155;">
            <form method="POST" action="{{ route('admin.social.posts.destroy', $post) }}" onsubmit="return confirm('Excluir este post?')">
                @csrf @method('DELETE')
                <button style="background:#ef444420;color:#ef4444;border:1px solid #ef4444;padding:.5rem 1rem;border-radius:.375rem;cursor:pointer;font-size:.85rem;">Excluir Post</button>
            </form>
        </div>
    </div>
</x-layouts.app>

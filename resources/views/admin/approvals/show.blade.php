...
<div style="margin-bottom:24px;display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
        ...
        <h1 style="font-size:1.4rem;font-weight:700;color:#f1f5f9;margin-top:8px;">
...
<div style="display:grid;grid-template-columns:2fr 1fr;gap:20px;">
    ...
    <div>
        ...
        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:14px;padding:24px;margin-bottom:20px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    ...
                    <div style="font-size:.875rem;color:#e2e8f0;">{{ $approvalRequest->client?->name ?? '—' }}</div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div style="font-size:.875rem;color:{{ $approvalRequest->isOverdue() ? '#f87171' : '#e2e8f0' }};">
                        {{ $approvalRequest->due_at ? $approvalRequest->due_at->format('d/m/Y H:i') : '—' }}
                        @if($approvalRequest->isOverdue()) <span style="color:#f87171;">(vencida)</span> @endif
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div style="font-size:.875rem;color:#f87171;">{{ $approvalRequest->rejectedBy?->name ?? '—' }} em {{ $approvalRequest->rejected_at?->format('d/m/Y H:i') }}</div>
                </div>
                @endif
            </div>
            ...
            <div style="margin-top:16px;">
                ...
                <div style="font-size:.875rem;color:#e2e8f0;background:#0a1628;border-radius:8px;padding:12px;">
        ...
        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:14px;padding:24px;margin-bottom:20px;">
            <h2 style="font-size:.9rem;font-weight:700;color:#f1f5f9;margin-bottom:16px;">Tomar Decisão</h2>
            <div style="margin-bottom:12px;">
                <label style="display:block;font-size:.8rem;font-weight:600;color:#94a3b8;margin-bottom:6px;">Observações (opcional)</label>
                <textarea id="notesField" rows="2" style="width:100%;background:#0a1628;border:1px solid #1e293b;border-radius:8px;padding:10px 12px;color:#e2e8f0;font-size:.875rem;resize:vertical;" placeholder="Adicione um comentário sobre sua decisão..."></textarea>
            </div>
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
                ...
                <form method="POST" action="{{ route('admin.approvals.cancel', $approvalRequest->id) }}" style="display:inline;">
                    ...
                    <button type="submit" onclick="document.getElementById('notesCancel').value=document.getElementById('notesField').value" style="background:#1e293b;color:#94a3b8;font-size:.8rem;font-weight:600;padding:9px 18px;border-radius:8px;border:none;cursor:pointer;">Cancelar</button>
                </form>
            </div>
        </div>
        @endif
    ...
    <div>
        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:14px;padding:24px;">
            <h2 style="font-size:.9rem;font-weight:700;color:#f1f5f9;margin-bottom:16px;">Histórico de Ações</h2>

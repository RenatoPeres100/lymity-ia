...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:.75rem;padding:1.5rem;max-width:640px;">
    <form method="POST" action="{{ route('admin.seo.clusters.update', $seoCluster) }}">
        ...
        <div style="margin-bottom:1rem;">
            ...
            <input type="text" name="name" value="{{ old('name', $seoCluster->name) }}" required style="width:100%;border:1px solid #d1d5db;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;box-sizing:border-box;">
        </div>
        ...
        <div style="margin-bottom:1rem;">
            ...
            <input type="text" name="main_keyword" value="{{ old('main_keyword', $seoCluster->main_keyword) }}" required style="width:100%;border:1px solid #d1d5db;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;box-sizing:border-box;">
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem;">
            <div>
                ...
                <select name="client_id" style="width:100%;border:1px solid #d1d5db;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;">
                    ...
                    <option value="{{ $client->id }}" {{ old('client_id', $seoCluster->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="status" style="width:100%;border:1px solid #d1d5db;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;">
                    <option value="active" {{ old('status', $seoCluster->status) == 'active' ? 'selected' : '' }}>Ativo</option>
                    <option value="paused" {{ old('status', $seoCluster->status) == 'paused' ? 'selected' : '' }}>Pausado</option>
                    <option value="archived" {{ old('status', $seoCluster->status) == 'archived' ? 'selected' : '' }}>Arquivado</option>
                </select>
            </div>
        </div>
        ...
        <div style="margin-bottom:1.5rem;">
            ...
            <textarea name="description" rows="3" style="width:100%;border:1px solid #d1d5db;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;box-sizing:border-box;">{{ old('description', $seoCluster->description) }}</textarea>
        </div>

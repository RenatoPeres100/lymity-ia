...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:28px;">
    <div>
        <h1 style="font-size:1.4rem;font-weight:700;color:#0f172a;margin-bottom:4px;">Agendamentos IA</h1>
        ...
        <tbody>
            ...
            <tr style="border-bottom:1px solid #f1f5f9;transition:background .15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                <td style="padding:12px 16px;">
                    <div style="font-size:.875rem;font-weight:600;color:#334155;">{{ $schedule->employee?->name ?? '—' }}</div>
                ...
                <td style="padding:12px 16px;">
                    <span style="background:{{ $schedule->active ? '#dcfce7' : '#f1f5f9' }};color:{{ $schedule->active ? '#166534' : '#64748b' }};border:1px solid {{ $schedule->active ? '#86efac' : '#e2e8f0' }};font-size:.7rem;font-weight:600;padding:3px 10px;border-radius:10px;">
                ...
                <td style="padding:12px 16px;text-align:right;">
                    <div style="display:flex;gap:8px;justify-content:flex-end;">
                        <form method="POST" action="{{ route('admin.ai-schedules.toggle', $schedule) }}">@csrf
                            <button type="submit" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;font-size:.75rem;padding:5px 12px;border-radius:6px;cursor:pointer;">
                                {{ $schedule->active ? 'Pausar' : 'Ativar' }}
                            </button>
                        </form>
                        <a href="{{ route('admin.ai-schedules.edit', $schedule) }}" style="background:#eff6ff;color:#4f46e5;border:1px solid #e0e7ff;font-size:.75rem;padding:5px 12px;border-radius:6px;text-decoration:none;">Editar</a>

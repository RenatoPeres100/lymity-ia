...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:28px;">
    <div>
        <h1 style="font-size:1.4rem;font-weight:700;color:#0f172a;margin-bottom:4px;">Editar Agendamento</h1>
...
<div class="table-wrapper" style="padding:28px;max-width:600px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
        <div>
            ...
            <input type="number" name="work_minutes_per_run" value="{{ old('work_minutes_per_run',$aiSchedule->work_minutes_per_run) }}" min="1" max="480" style="width:100%;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;color:#334155;font-size:.875rem;">
        </div>
        <div>
            <label style="font-size:.78rem;color:#475569;display:block;margin-bottom:6px;">Máx. tarefas por execução</label>
            <input type="number" name="max_tasks_per_run" value="{{ old('max_tasks_per_run',$aiSchedule->max_tasks_per_run) }}" min="1" max="20" style="width:100%;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;color:#334155;font-size:.875rem;">
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
        ...
        <div>
            ...
            <input type="time" name="time_of_day" value="{{ old('time_of_day', $aiSchedule->time_of_day) }}" style="width:100%;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;color:#334155;font-size:.875rem;">
        </div>
    </div>

...
<style>
  ...
  .header{background:linear-gradient(135deg,#1e1b4b 0%,#312e81 50%,#1e40af 100%);padding:36px 40px 28px;text-align:center;}
  .logo{font-size:22px;font-weight:800;color:#ffffff;letter-spacing:-.02em;}
  .logo span{color:#818cf8;}
  .header-subtitle{font-size:13px;color:#c7d2fe;margin-top:6px;letter-spacing:.04em;text-transform:uppercase;}
  .badge{display:inline-block;background:rgba(255,255,255,.15);color:#e0e7ff;font-size:11px;font-weight:600;padding:4px 12px;border-radius:20px;margin-top:12px;letter-spacing:.06em;text-transform:uppercase;}
  ...
  .desc{font-size:14px;color:#475569;line-height:1.65;margin-top:12px;}
...
<div class="wrapper">
  ...
  <div class="body">
    ...
    <div class="alert-bar">
      <p>Há um conteúdo aguardando sua aprovação. Revise com antecedência para que a publicação siga no prazo planejado.</p>
    </div>
    ...
    <div class="card">
      ...
      <div class="desc">{{ $approval->description }}</div>
      @endif
      ...
      <div class="meta-grid">
        ...
        <div class="meta-item">
          ...
          <span>{{ $approval->aiEmployee->name }}</span>
        </div>
        @endif
        <div class="meta-item">
          <label>Criado em</label>
          <span>{{ $approval->created_at->format('d/m/Y \à\s H:i') }}</span>
        </div>
        @if($approval->due_at)
        <div class="meta-item">
          <label>Prazo</label>
          <span style="color:{{ $approval->due_at->isPast() ? '#dc2626' : '#059669' }};">
            {{ $approval->due_at->format('d/m/Y \à\s H:i') }}
          </span>
        </div>
        @endif
        ...
        <div class="meta-item">
          ...
          <span>{{ $approval->approvable->scheduled_at->format('d/m/Y \à\s H:i') }}</span>
        </div>
        @endif
      </div>
    </div>
  ...
  <div class="footer">
    <p>
      Olá, <strong>{{ $recipient->name }}</strong>. Você recebe este e-mail porque tem permissão para aprovar conteúdos na Lymity IA.<br>
